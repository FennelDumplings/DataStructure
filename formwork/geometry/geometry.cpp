...
bool LinesegInsidePolygon(int vcount,Point polygon[],LineSeg l)
{
    ...
    for(i=0;i<top-1;i++) /* 冒泡排序，x坐标小的排在前面；x坐标相同者，y坐标小的排在前面 */
    {
        for(j=i+1;j<top;j++)
        {
            if( PointSet[i].x>PointSet[j].x || fabs(PointSet[i].x-PointSet[j].x)<EPS && PointSet[i].y>PointSet[j].y )
            {
                tmp=PointSet[i];
...
{
    Point d;
    if(abs(dotmultiply(a,b,c))<EPS) // 说明c点是直角拐角处
    {
        d.x=a.x+b.x-c.x;
        d.y=a.y+b.y-c.y;
    }
    if(abs(dotmultiply(a,c,b))<EPS) // 说明b点是直角拐角处
    {
        d.x=a.x+c.x-b.x;
        d.y=a.y+c.y-b.x;
    }
    if(abs(dotmultiply(c,b,a))<EPS) // 说明a点是直角拐角处
    {
        d.x=c.x+b.x-a.x;
        d.y=c.y+b.y-a.y;
    }
